module Paperclip::FormHelper
  ...
  def paperclip_file_field(object_name, method, options = {})
    ...
    if is_s3_enabled?(object,method)
      ...
      options[:direct]=object.send(method).instance_variable_get('@styles').present? ? "true" : "false"
      # initialize the hidden form fields
      hidden_form_fields = {
      ...
      "<div class='paperclip_field'>
        ...
        ActionView::Helpers::InstanceTag.new(object_name, method, self, hidden_options.delete(:object)).to_input_field_tag("file", hidden_options).html_safe + "" +
        ActionView::Helpers::InstanceTag.new(object_name, method, self, options.delete(:object)).to_input_field_tag("file", options).html_safe + "" +
        "</div></div>
          <div id='#{error_name}' class='s3_error'>File not supported or larger in size</div>
      "
    else
  ...
  def paperclip_file_field_tag(object_name, method, options = {})
    ...
    if is_s3_enabled?(object,method)
      ...
      "<div class='paperclip_field'>
        ...
        text_field_tag("#{object_name}[#{method}]", nil, hidden_options.update("type" => "file")).html_safe + "" +
        text_field_tag("#{object_name}[#{method}]", nil, options.update("type" => "file")) + "" +
        "</div></div>
         <div id='#{error_name}' class='s3_error'>File not supported or larger in size</div>
      "
    else
