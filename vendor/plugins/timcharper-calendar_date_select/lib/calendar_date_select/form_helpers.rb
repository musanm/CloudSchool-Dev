...
#      "<div class='calendar_field_tag'>"+text_field_tag(name, value, options)+"<div class='calendar_label'></div></div>"
  ...
  def calendar_date_select(object, method, options={})
    ...
    formatted_date = options[:value].present? ? format_date(options[:value].length > 10 ? options[:value].to_datetime : options[:value].to_date,:format=>:long) : ''
    options[:class] = (options[:class].present? ? options[:class] : '')+' calendar_field'
  ...
  private
    ...
    def calendar_date_select_output(input, image, options = {}, javascript_options = {}, formatted_date = nil)
      out = input
      javascript_options['locale_months'] = "'#{I18n.t('date.month_names').compact.join(',')}'"
      javascript_options['date_format'] = date_format
      css_classes = options[:class].present? ? options[:class].split(' ') : []
      css_classes << 'calendar_label'
      css_classes.delete('calendar_field')
      options[:class] = css_classes.join(' ')
      options[:readonly] = 'readonly' unless options[:readonly].present?
      if javascript_options[:embedded]
        ...
        out << text_field_tag('', formatted_date, options) #{:class=>"calendar_label",:readonly=>'readonly'})
        out << image_tag(image,
