module Searchkick
  module Reindex
    ...
    def reindex(options = {})
      ...
      new_name = alias_name + "_" + Time.now.strftime("%Y%m%d%H%M%S#{(Time.now.to_f*1000.0).to_i.to_s[10..12]}")
    ...
    def clean_indices
      ...
      indices = Hash[ all_indices.select{|k, v| v["aliases"].empty? && k =~ /\A#{Regexp.escape(searchkick_index.name)}_\d{14,17}\z/ } ]
    ...
    def searchkick_import(index)
      ...
      if scope.respond_to?(:find_in_batches)
        scope.find_all_for_searchkick_reindex :batch_size => batch_size do |batch|
    ...
    def searchkick_index_options
      ...
      if options[:mappings] and !options[:merge_mappings]
        ...
        mapping_options = Hash[
          [:autocomplete, :suggest, :text_start, :text_middle, :text_end, :word_start, :word_middle, :word_end].map{|type| [type, (options[type] || []).map(&:to_s)] }
        ]
