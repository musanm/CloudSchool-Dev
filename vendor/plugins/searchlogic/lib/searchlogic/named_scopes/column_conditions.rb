module Searchlogic
  module NamedScopes
    ...
    module ColumnConditions
      ...
      DOTTED_CONDITIONS = [:dotted_equals,:dotted_like,:dotted_begins_with]
      ...
      private
        ...
        def condition_details(method_name)
          ...
          conditions_matcher = (PRIMARY_CONDITIONS + ALIAS_CONDITIONS + DOTTED_CONDITIONS).join("|")
        ...
        def create_condition(name)
          ...
          if details = condition_details(name)
            if (PRIMARY_CONDITIONS+DOTTED_CONDITIONS).include?(details[:condition].to_sym)
        ...
        def create_primary_condition(column_name, condition)
          ...
          when /^dotted_equals/
            scope_options(condition, column, "#{table_name}.#{column.name} = REPLACE(?, '&dot;', '.')", :skip_conversion => skip_conversion)
          when /^dotted_begins_with/
            scope_options(condition, column, "#{table_name}.#{column.name} #{match_keyword} REPLACE(?, '&dot;', '.')", :skip_conversion => skip_conversion, :value_modifier => :begins_with)
          when /^dotted_like/
            scope_options(condition, column, "#{table_name}.#{column.name} #{match_keyword} REPLACE(?, '&dot;', '.')", :skip_conversion => skip_conversion, :value_modifier => :like)
          when /^equals/
