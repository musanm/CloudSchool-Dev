...
function ts_resortTable(lnk, clid) {
    ...
    var row_span,row_index,ar_index;
    var slRows = new Array();
    ...
    for (k=0;k<t.tBodies.length;k++) {
        if (!thead) {
            ...
            for (var j=1;j<t.tBodies[k].rows.length;j++) {
                oldRows[j-1] = t.tBodies[k].rows[j];
                if(ar_index == j-1)
                {
                    newRows.push(t.tBodies[k].rows[j]);
                    ar_index += parseInt(t.tBodies[k].rows[j].cells[SORT_COLUMN_INDEX].getAttribute('rowspan'));
                }
            }
        } else {
    ...
    for (i=0; i<newRows.length; i++) {
        if (!newRows[i].className || (newRows[i].className && (newRows[i].className.indexOf('sortbottom') == -1))) {
            ...
            row_span = parseInt(newRows[i].cells[SORT_COLUMN_INDEX].getAttribute('rowspan'));
            for(s=1;s<row_span;s++)
            {
                row_index = get_element_index(oldRows,newRows[i]) + s;
                t.tBodies[0].appendChild(oldRows[row_index]);
            }
        }
    }
    ...
    for(i=0;i<serial_columns.length;i++)
    {
        ...
        {
            if(t.tBodies[0].rows[j].cells[0].className.indexOf('sinumber') != -1)
            {
                slRows.push(t.tBodies[0].rows[j]);
            }
        }
        ts_make_serial_no(slRows,serial_columns[i]);
    }
}

function ts_make_serial_no(trows,cindex){
    //function for keeping serial numbers as it is
    page = window.location.search.match(/(\?|&)page\=([^&]*)/);
    for(i=1;i<trows.length;i++){
        if(page != null)
        {
            if(parseInt(page[2]) >= 1)
            {
                trows[i].cells[cindex].innerHTML= i + (parseInt(page[2]) - 1) * 30;
            }
        }else{
...
function get_element_index(arr,elm)
{
