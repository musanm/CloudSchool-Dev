...
class UserController < ApplicationController
  ...
  before_filter :login_required, :except => [:forgot_password, :login, :student_in, :student_out, :student_attendances, :set_new_password, :reset_password]
  ...
  before_filter :check_if_loggedin, :only => [:login, :student_in, :student_out, :student_attendances]
  #  around_filter :cache_quick_links, :only => [:show_quick_links]
  include AutosuggestMenuHelper

  # def student_in
  #   student_attendance = StudentAttendance.new(:in_time => params[:in_time].to_time, :student_id => params[:id] )
  ...
  # def student_out
  #   out_time = params[:out_time].to_time
  #   student_attendance = StudentAttendance.find(:all, :conditions => {:student_id => params[:id] , :in_time =>  (out_time.beginning_of_day..out_time.end_of_day)}).last rescue nil
  #   if student_attendance.present?
  #     if student_attendance.update_attributes(:out_time => out_time)
  #       respond_to do |format|
  ...
  def student_attendances
    ...
    respond_to do |format|
      msg = { :status => "true", :message => "Success!", :data => { :student_attendances => student_attendances } }
      format.json  { render :json => { :student_attendances =>  StudentAttendance.all.map{|c| {:id => c.id, :student_id => c.student_id, :in_time => c.in_time, :out_time => c.out_time} } } }
    end
  end
