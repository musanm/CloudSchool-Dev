...
class CalendarController < ApplicationController
  ...
  def index
    ...
    if @user.employee? and !privilege.include?("EventManagement")
      @events = Event.all(:conditions =>["(start_date BETWEEN ? AND ?) or (end_date BETWEEN ? AND ?) or (? BETWEEN start_date AND end_date) or (? BETWEEN start_date AND end_date)",first_day,last_day,first_day,last_day,first_day,last_day],:include=>[:origin,:employee_department_events])
    end
    if @user.student? or @user.parent?
      @events = Event.all(:conditions =>["(start_date BETWEEN ? AND ?) or (end_date BETWEEN ? AND ?) or (? BETWEEN start_date AND end_date) or (? BETWEEN start_date AND end_date)",first_day,last_day,first_day,last_day,first_day,last_day],:include=>[:origin,:batch_events])
    end
    load_notifications
  end
