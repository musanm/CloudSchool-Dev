...
class CceGradeSetsController < ApplicationController
  ...
  def create
    ...
    if @grade_set.save
      flash[:notice]=t('grade_set_created')
      @grade_sets=CceGradeSet.all
    else
      @error=true
    end
  ...
  def update
    ...
    if @grade_set.save
      flash[:notice]=t('grade_set_updated')
      @grade_sets=CceGradeSet.all
    else
      @error=true
    end
  ...
  def destroy
    ...
    if @grade_set.observation_groups.empty?
      if @grade_set.destroy
        ...
        flash[:notice]=t('dependent_data_exists')
      end
    else
      flash[:warn_notice]=t('associated_data_exists')
    end
  ...
  def destroy_grade
    ...
    if @grade.destroy
      ...
      flash[:notice]=t('no_delete_grade')
    end
