...
class FinanceFeeCategory < ActiveRecord::Base
  ...
  def check_fee_collection(batch_id)
    categories=FinanceFeeCategory.find(:all, :include => :category_batches, :conditions => ["name= ? and description= ? and is_deleted=#{false}", "#{name}", "#{description}"]).uniq.map { |d| d if d.batch_id==batch_id }.compact
    if categories.present?
      category_id = FinanceFeeCategory.find(:first, :conditions => ["name=? and batch_id=#{batch_id} and is_deleted='#{false}'", "#{name}"]).id
    else
      category_id=id
    end
...
#  end
  ...
  def check_category_name_exists(batch)
    ds=id.to_i
    if batch.finance_fee_categories.all(:conditions => ["name = ? and finance_fee_categories.id <> #{ds}", "#{name}"]).present?
